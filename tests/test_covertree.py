...
def check_invariant(tree):
    ...
    while level > tree.minlevel:
        ...
        for p in parent:
            ...
            ok_(all([tree.distance(p.data,c.data) <= 2 ** level for c in  p.getChildren(level - 1)]))
        
        children = sum([d.getChildren(level - 1) for d in parent], [])

        # Each children is distant from other children at least 
        [[ok_(tree.distance(p.data, q.data) >= 2 ** (level - 1)) for p in children if p != q] for q in children]
...
def test_query_ball():
    ...
    ref = (periodic_distance([1.0, 1.0, 1.0], positions, np.array([10, 10, 10])) < 3.0).nonzero()[0]
