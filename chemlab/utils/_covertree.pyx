...
cdef class PeriodicMetric:
    
    cdef double[:] _cell_lengths
    cdef int ndim
    
    def __init__(self, cell_lengths):
        self._cell_lengths = np.array(cell_lengths).astype('float64') 
        self.ndim = len(cell_lengths)
    
    @cython.boundscheck(False)
    @cython.initializedcheck(False)
    cpdef double distance(self, np.ndarray[double, ndim=1] a, np.ndarray[double, ndim=1] b):
        """Periodic distance implemented in C for speed"""
        cdef double dr
        cdef int sign
        cdef double abs_
        
        cdef double dist2 = 0.0
        
        for i in range(self.ndim):
            dr = abs(b[i] - a[i])
            
            if abs_ > 0.5 * self._cell_lengths[i]:
                dr = self._cell_lengths[i] - abs_
...
class cCoverTree:
    ...
    def __init__(self, metric, metric_args={}):
        #print type(PeriodicMetric)
        self.root = None
        self.maxlevel = 0
        self.minlevel = -1
        self.nodes = {}
        ...
        if metric == "periodic":
            self._metric = PeriodicMetric(metric_args['cell_lengths'])
            self.distance = self._metric.distance
        else:
    ...
    def insert(self, point):
        point = np.array(point).astype('float64')
        ...
        if self.root is None:
            ...
            return self.root
        elif self._metric.distance(self.root.data, point) > 2 ** self.maxlevel:
            # In this case, the root is too low, we need to bring up the maxlevel
            root_dist = self._metric.distance(self.root.data, point)
    ...
    def _insert(self, point, cover_set, level):
        ...
        children = np.array(sum((n.getChildren(level - 1) for n in cover_set),
                                []))
        children_dist = np.array([self._metric.distance(point, c.data) for c in
                                  children])
        children_dist_min = min(children_dist)

        parent_dist = np.array([self._metric.distance(point, p.data) for p in cover_set
                                ])
